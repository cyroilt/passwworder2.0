...
def cnd() :
    global message1,message2,en,en1,ens,a1,n,my_file,messagenam,ennam,messages
    message1 = ""
    message2 = ""
    messagenam = ""
    messages = ""
    ...
    ennam.pack()
    ens = Entry(update,font=("Verdana", 15 ), bg="silver", fg="black",textvariable=messages)
    ens.pack()
    en.insert(0, "Name")
    en1.insert(0, "Password")
    ennam.insert(0, "Save as")
    ens.insert(0, "Site url")
...
def rnd () :
    ...
    btn1.pack()
def ready () :
    global ld,ld1,lds,rom,m1,m2,ms
    a3 = en2.get()
    n= a3
    print(n)
    ...
    sb = Button(rom, text="go to site", font=("Verdana", 10),bg="grey",padx="10",pady="20", command = gs)
    nb.pack()
    pb.pack()
    sb.pack()
...
def createinfo ():
     infod=Tk()
     inl = Label(infod,text = "A Little Qubit corp. 2019. © all rights reserved")
     inl.pack()
def cl ():
    boot.destroy()
def gs ():
    ...
    webbrowser.get('chrome').open_new_tab(ms)
boot = Tk()
boot.title("passwworder 2.0")
boot.configure(background='red')
boot.geometry("+500+250")
boot.overrideredirect(True)
boot.wm_attributes("-transparentcolor", "grey")
boot.wm_attributes("-transparentcolor", "red")
...
photocl= PhotoImage (file='log\\images\\close.png')
...
butinf.pack()
bucl = Button(boot,image=photocl,bg="grey",padx="10",pady="20", command = cl)
bucl.pack()
bucl.place(x=240,y=0)
boot.mainloop()
